...
def unicode(s, errors="strict"):
    """Convert sandbox output to text on the supported Python 3 runtime."""
    if isinstance(s, str):
        return s
...
class Jail(object):
    ...
    def __init__(self, working_directory):
        ...
        self.chroot_cmd = (
            "sudo -u {0} schroot -u {0} -c {0} -d {1} -- jailguard.py ".format(
    ...
    def __del__(self):
        if self.locked:
            raise SandboxError(
                "Jail object for %s freed without being released" % (self.name)
            )
    ...
    def release(self):
        ...
        if os.system("sudo umount %s" % (os.path.join(self.base_dir, "root"),)):
            raise SandboxError("Error returned from umount of jail %d" % (self.number,))
    ...
    def _prepare_with(self, command_dir):
        if os.system("%s c %d" % (self.jchown, self.number)) != 0:
            raise SandboxError(
                "Error returned from jail_own c %d in prepare" % (self.number,)
            )
        scratch_dir = os.path.join(self.base_dir, "scratch")
        if os.system("rm -rf %s" % (scratch_dir,)) != 0:
            raise SandboxError(
                "Could not remove old scratch area from jail %d" % (self.number,)
            )
        ...
        if os.system("sudo mount %s" % (os.path.join(self.base_dir, "root"),)):
            raise SandboxError(
                "Error returned from mount of %d in prepare" % (self.number,)
            )
        if os.system("%s j %d" % (self.jchown, self.number)) != 0:
            raise SandboxError(
                "Error returned from jail_own j %d in prepare" % (self.number,)
            )
    ...
    def _signal(self, signal):
        ...
        result = subprocess.call(
            "sudo -u {0} kill -{1} -1".format(self.name, signal), shell=True
        )
        if result != 0:
            raise SandboxError(
                "Error returned from jail %s sending signal %s" % (self.name, signal)
            )
    ...
    def pause(self):
        ...
        if item[1] != "STOP" and item[1] is not None:
            raise SandboxError("Bad response from jailguard after pause, %s" % (item,))
    ...
    def resume(self):
        ...
        if item[1] != "CONT" and item[1] is not None:
            raise SandboxError("Bad response from jailguard after resume, %s" % (item,))
...
class House:
    ...
    def start(self, shell_command):
        ...
        try:
            ...
            raise SandboxError("Failed to start {0}".format(shell_command))
        self._is_alive = True
        stdout_monitor = Thread(
            target=_monitor_file, args=(self.command_process.stdout, self.stdout_queue)
        )
        stdout_monitor.daemon = True
        stdout_monitor.start()
        stderr_monitor = Thread(
            target=_monitor_file, args=(self.command_process.stderr, self.stderr_queue)
        )
