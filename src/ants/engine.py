...
def unicode(s):
    """Return text unchanged on the supported Python 3 runtime."""
    return s
...
def run_game(game, botcmds, options):
    ...
    try:
        ...
        for b, bot in enumerate(botcmds):
            bot_cwd, bot_cmd = bot
            sandbox = get_sandbox(bot_cwd, secure=options.get("secure_jail", None))
        ...
        for turn in range(turns + 1):
            ...
            for group_num in range(0, len(bot_list), simul_num):
                ...
                moves, errors, status = get_moves(game, pbots, pnums, time_limit, turn)
            ...
            for b in bots_eliminated:
                ...
                status_line = STATUS_LINE % " ".join(
                    map(str, game.order_for_player(b, bot_status))
                )
                status_line += PLAYER_TURNS_LINE % " ".join(
            ...
            if verbose_log:
                ...
                if turn % 50 == 0:
                    ...
                    for key in stat_keys:
                        values = stats[key]
                        verbose_log.write(
                            " {0:^{1}}".format(key, max(len(key), len(str(values))))
                        )
                    verbose_log.write("\n")
                verbose_log.write(s)
                for key in stat_keys:
                    ...
                    if values is None:
                        values_str = "None"
                        width = max(len(key), len(values_str))
                    else:
...
def get_moves(game, bots, bot_nums, time_limit, turn):
    ...
    #   or when time is up
    while (
        sum(bot_finished) < len(bot_finished) and time.time() - start_time < time_limit
    ):
        time.sleep(0.01)
        for b, bot in enumerate(bots):
            if bot_finished[b]:
