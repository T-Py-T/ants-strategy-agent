...
import operator
from src.ants.game import Game
...
class Ants(Game):
    def __init__(self, options=None):
        ...
        if type(self.food_visible) in (list, tuple):
            self.food_visible = randrange(
                self.food_visible[0], self.food_visible[1] + 1
            )
        ...
        self.initial_ant_list = sorted(
            self.current_ants.values(), key=operator.attrgetter("owner")
        )
        ...
        self.switch = [
            [None] * self.num_players + list(range(-5, 0))
    ...
    def parse_map(self, map_text):
        ...
        for line in map_text.split("\n"):
            ...
            if key == "cols":
                ...
                if num_players < 2 or num_players > 10:
                    raise ValueError("map", "player count must be between 2 and 10")
            elif key == "score":
                ...
                if len(value) != width:
                    raise ValueError(
                        ...
                        "Got %s, expected %s." % (row, len(value), width),
                    )
                for col, c in enumerate(value):
                    if c in ant_list:
                        ...
                        raise ValueError("map", "Invalid character in map: %s" % c)
                row += 1
        ...
        if hive and len(hive) != num_players:
            raise ValueError(
                "map",
                "Incorrect score count.  Expected %s, got %s"
                % (num_players, len(hive)),
            )

        if height != row:
            raise ValueError(
                "map", "Incorrect number of rows.  Expected %s, got %s" % (height, row)
            )
        ...
        if not self.scenario:
            for hill, count in hill_count.items():
                if count == 0:
                    raise ValueError("map", "Player %s has no starting hills" % hill)
        ...
        for ant in self.current_ants.values():
            if not ant.orders:
                ...
                if order in AIM:
                    ...
                    self.update_vision_ant(
                        ant, self.vision_offsets_cache[order][-1], -1
                    )
    ...
    def get_perspective(self, player=None):
        ...
        for row, squares in enumerate(self.map):
            ...
            for col, square in enumerate(squares):
                if player is None or self.vision[player][row][col]:
            ...
            if v[row][col] or (update_type == "d" and update[-1] == player):
                ...
                if update_type in ["a", "d", "h"]:
                    ...
                    if self.switch[player][update[-1]] is None:
                        self.switch[player][update[-1]] = (
                            self.num_players - self.switch[player].count(None)
                        )
    ...
    def get_state_changes(self):
        ...
        # current food
        changes.extend(sorted(["f", row, col] for row, col in self.current_food))
        # ants killed this turn
        changes.extend(
            sorted(["d", ant.loc[0], ant.loc[1], ant.owner] for ant in self.killed_ants)
        )
    ...
    def do_gather(self):
        ...
        """
        # gather food
        for f_loc in self.current_food.copy():
    ...
    def do_spawn(self):
        ...
        for player in range(self.num_players):
            ...
            for hill in player_hills:
                ...
                if self.hive_food[player] > 0 and hill.loc not in self.current_ants:
    ...
    def add_food(self, loc):
        ...
        if self.map[loc[0]][loc[1]] != LAND:
            raise ValueError("Add food error", "Food already found at %s" % (loc,))
    ...
    def remove_food(self, loc, owner=None):
        ...
        try:
            ...
            raise ValueError("Remove food error", "Food not found at %s" % (loc,))
    ...
    def kill_ant(self, ant, ignore_error=False):
        ...
        try:
            ...
            if not ignore_error:
                raise ValueError("Kill ant error", "Ant not found at %s" % (loc,))
    ...
    def do_attack_focus(self):
        ...
        nearby_enemies = {}
        for ant in self.current_ants.values():
            nearby_enemies[ant] = self.nearby_ants(
                ant.loc, self.attackradius, ant.owner
            )
        ...
        for ant in self.current_ants.values():
            ...
            min_enemy_weakness = min(
                len(nearby_enemies[enemy]) for enemy in nearby_enemies[ant]
            )
    ...
    def place_food(self):
        ...
        for loc in self.pending_food.copy():
            if self.map[loc[0]][loc[1]] == LAND:
                self.add_food(loc)
                self.pending_food[loc] -= 1
    ...
    def map_similar(self, loc1, loc2, aim, player):
        ...
        for row in range(self.height):
            for col in range(self.width):
                ...
                mismatch = (
                    (ilk0 == 0 and ilk1 != player)
                    or (ilk0 > 0 and (ilk1 < 0 or ilk1 == player))
                    or (ilk0 < 0 and ilk1 != ilk0)
                )
                if mismatch:
                    return None
                if ilk0 >= 0:
    ...
    def get_map_symmetry(self):
        ...
        orientations = [[(hills[0].loc, 0, {i: i for i in range(self.num_players)})]]
        for player in range(1, self.num_players):
            player_hills = [
                hill for hill in self.hills.values() if hill.owner == player
            ]
            if len(player_hills) != len(hills):
                raise ValueError(
                    INVALID_MAP,
                    "This map is not symmetric.  Player 0 has {0} hills while player {1} has {2} hills.".format(
                        len(hills), player, len(player_hills)
                    ),
                )
            new_orientations = []
            for player_hill in player_hills:
                for aim in range(8):
                    ...
                    enemy_map = self.map_similar(
                        hills[0].loc, player_hill.loc, aim, player
                    )
                    if enemy_map != None:
            ...
            if len(orientations) == 0:
                raise ValueError(
                    INVALID_MAP,
                    "This map is not symmetric. Player {0} does not have an orientation that matches player 0".format(
                        player
                    ),
                )
        ...
        if len(valid_orientations) == 0:
            raise ValueError(INVALID_MAP, "There are no valid orientation sets")
        ...
        for hill in self.hills.values():
            ...
            while squares:
                ...
                for d in AIM.values():
                    n_loc = self.destination(c_loc, d)
                    if (
                        n_loc not in vision_squares
                        and self.map[n_loc[0]][n_loc[1]] != WATER
                        and self.distance(hill.loc, n_loc) <= self.viewradius
                    ):
                ...
                max_score = (
                    sum(
                        [
                            HILL_POINTS
                            for hill in self.hills.values()
                            if hill.killed_by is None and hill.owner != player
                        ]
                    )
                    + self.score[player]
                )
                        ...
                        min_score = (
                            sum(
                                [
                                    RAZE_POINTS
                                    for hill in self.hills.values()
                                    if hill.killed_by is None and hill.owner == opponent
                                ]
                            )
                            + self.score[opponent]
                        )
                        if (
                            self.score[player] < self.score[opponent]
    ...
    def start_game(self):
        ...
        if self.do_food != self.do_food_none:
            ...
            if self.food_start:
                starting_food = (
                    self.land_area // self.food_start
    ...
    def finish_game(self):
        ...
        if len(players) == 1:
            self.bonus[players[0]] += sum(
                [
                    HILL_POINTS
                    for hill in self.hills.values()
                    if hill.killed_by == None and hill.owner != players[0]
                ]
            )
    ...
    def calc_significant_turns(self):
        ...
        winning_bot = [
            p for p in range(len(self.score)) if self.score[p] == max(self.score)
        ]
    ...
    def do_moves(self, player, moves):
        ...
        orders, valid, ignored, invalid = self.validate_orders(
            player, orders, valid, ignored, invalid
        )
        self.orders[player] = orders
        return (
            valid,
            ["%s # %s" % ignore for ignore in ignored],
            ["%s # %s" % error for error in invalid],
        )
    ...
    def order_for_player(self, player, data):
        ...
        return [
            None if i not in s else data[s.index(i)]
    ...
    def get_stats(self):
        ...
        if self.cutoff_bot == FOOD:
            cutoff = "Food"
        elif self.cutoff_bot == LAND:
            cutoff = "-"
        else:
            cutoff = self.cutoff_bot
        stats["cutoff"] = cutoff
        ...
        stats["s_alive"] = [
            1 if self.is_alive(player) else 0 for player in range(self.num_players)
        ]
        stats["s_hills"] = [
            1 if player in self.remaining_hills() else 0
            for player in range(self.num_players)
        ]
        stats[CLIMB_STAT] = []
        for player in range(self.num_players):
                ...
                for opponent in range(self.num_players):
                    if player != opponent:
                        ...
                        if (
                            ...
                            found = 1
                            break
                stats[CLIMB_STAT].append(found)
            else:
                stats[CLIMB_STAT].append(0)
        return stats
...
def test_symmetry():
    ...
    for o_count, ant_aims in enumerate(ors):
        ...
        if len(sys.argv) > 2:
            test_map_name = (
                map_file_name
                + "".join([str(aim) for _, aim, __ in ant_aims])
                + ".replay"
            )
