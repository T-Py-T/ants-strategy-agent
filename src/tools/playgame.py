...
def main(argv):
    ...
    parser.add_option("-m", "--map_file", dest="map", help="Name of the map file")
    ...
    game_group = OptionGroup(
        parser, "Game Options", "Options that affect the game mechanics for ants"
    )
    ...
    game_group.add_option(
        "--scenario", dest="scenario", action="store_true", default=False
    )
    ...
    log_group = OptionGroup(
        parser, "Logging Options", "Options that control the logging"
    )
    ...
    log_group.add_option(
        "-R", "--log_replay", dest="log_replay", action="store_true", default=False
    ),
    log_group.add_option(
        "-S", "--log_stream", dest="log_stream", action="store_true", default=False
    ),
    log_group.add_option(
        "-I",
        "--log_input",
    ...
    try:
        if opts.profile:
            ...
            print(
                "Running profile and outputting to {0}".format(
                    prof_file,
                ),
                file=stderr,
            )
    ...
    for round in range(opts.rounds):
        ...
        if game.num_players != len(bots):
            if game.num_players > len(bots) and opts.fill:
                ...
                print(
                    "Incorrect number of bots for map.  Need {0}, got {1}".format(
        ...
        if (
            not opts.log_replay
            and not opts.log_stream
            and (opts.log_dir or opts.log_stdout)
        ):
        ...
        if opts.log_stream:
            if opts.log_dir:
                engine_options["stream_log"] = open(
                    os.path.join(opts.log_dir, "{0}.stream".format(game_id)), "w"
                )
            if opts.log_stdout:
        ...
        if opts.log_input and opts.log_dir:
            engine_options["input_logs"] = [
                open(
                    os.path.join(opts.log_dir, "{0}.bot{1}.input".format(game_id, i)),
                    "w",
                )
        ...
        if opts.log_output and opts.log_dir:
            engine_options["output_logs"] = [
                open(
                    os.path.join(opts.log_dir, "{0}.bot{1}.output".format(game_id, i)),
                    "w",
                )
                for i in range(bot_count)
            ]
                ...
                if opts.log_stdout:
                    engine_options["error_logs"] = [
                        Tee(
                            ...
                            open(
                                os.path.join(
                                    opts.log_dir, error_log_pattern.format(game_id, i)
                                ),
                                "w",
                            ),
                        )
            ...
            try:
                ...
                if ranks:
                    ...
                    if len(top_players) == 1:
                        ...
                        winner = "tie({0})".format(
                            ",".join("player_%d" % i for i in top_players)
                        )
                ...
                for i, name in enumerate(player_names):
                    ...
                    fields.append(
                        "player_{0}={1}:rank={2},score={3},status={4}".format(
        ...
        if replay_path:
            if opts.nolaunch:
                ...
                if opts.html_file == None:
                    visualizer.visualize_locally.launch(
                        replay_path, generated_path="replay.{0}.html".format(game_id)
                    )
                else:
                    visualizer.visualize_locally.launch(
