...
class Grid:
    ...
    def rotationally_symmetric_grid(
        self,
        no_players,
        min_dimensions,
        max_dimensions,
    ...
    def pick_tile_dimensions(self):
        ...
        for _ in range(200000):
            ...
            if (
                self.rows <= self.max_dimensions
                and self.cols <= self.max_dimensions
                and self.no_players
                == lcm(
                    self.rows / gcd(self.row_t, self.rows),
                    self.cols / gcd(self.col_t, self.cols),
                )
                and self.no_players >= min_players
                and self.no_players <= max_players
                and self.rows / gcd(self.row_t, self.rows)
                == self.cols / gcd(self.col_t, self.cols)
                and self.row_t % (2 * self.block_size) == 0
                and self.col_t % (2 * self.block_size) == 0
                and self.is_valid_start()
            ):
                return True
        return False
    ...
    def pick_rotational_dimensions(self):
        ...
        for _ in range(100):
            ...
            if (
                (self.no_players == 2 and self.r_sym_type > 3)
                or (self.no_players == 4 and self.r_sym_type > 1)
                or self.no_players == 8
            ):
                self.cols = self.rows
    ...
    def get_translate_loc(self, loc, t_amount):
        return [(loc[0] + t_amount[0]) % self.rows, (loc[1] + t_amount[1]) % self.cols]
            ...
            for _ in range(100):
                ...
                if self.squares[c_block[0][0]][c_block[0][1]] == "%":
                    for d in directions:
                        n_block = self.get_adjacent_block(c_block, d)
                        if (
                            self.is_basis_block[n_block[0][0]][n_block[0][1]]
                            and self.squares[n_block[0][0]][n_block[0][1]] == "."
                        ):
                            block_found = True
                            break
                    if block_found:
    ...
    def add_extra_land_locs(self):
        ...
        while stack:
            ...
            for d in directions:
                ...
                if not visited[n_loc[0]][n_loc[1]]:
                    if (
                        self.is_basis_loc[n_loc[0]][n_loc[1]]
                        and self.squares[n_loc[0]][n_loc[1]] == "%"
                    ):
                        w_locs.append(n_loc)
...
def main(argv):
    ...
    if (
        opts.no_players < min_players and opts.no_players != -1
    ...
    if opts.symmetry == "rotational":
        if (
            opts.no_players != -1
            and opts.no_players != 2
            and opts.no_players != 4
            and opts.no_players != 8
        ):
        ...
        if opts.rotational_symmetry != -1:
            if (
                (
                    opts.no_players == 2
                    and (opts.rotational_symmetry < 1 or opts.rotational_symmetry > 5)
                )
                or (
                    opts.no_players == 4
                    and (opts.rotational_symmetry < 1 or opts.rotational_symmetry > 3)
                )
    ...
    if opts.symmetry == "rotational":
        ...
        if (opts.no_players == -1 or opts.no_players % 2 == 0) and random.randint(0, 5):
            grid.symmetry = "rotational"
        else:
            grid.symmetry = "tile"
    ...
    if grid.symmetry == "rotational":
        if not grid.rotationally_symmetric_grid(
            opts.no_players,
            opts.min_dimensions,
            opts.max_dimensions,
            ...
            print(
                "Failed to create a valid rotationally symmetric grid with",
                opts.no_players,
                "players",
            )
            return
        ...
        if not grid.tile_symmetric_grid(
            opts.no_players,
            opts.min_dimensions,
            opts.max_dimensions,
            opts.min_starting_distance,
            ...
            print(
                "Failed to create a valid tile symmetric grid with",
                opts.no_players,
                "players and block size",
                grid.block_size,
            )
            return
