...
COLOR = {LAND: LAND_COLOR, BARRIER: BARRIER_COLOR, UNSEEN: LAND_COLOR}
for i in range(26):
    COLOR[i] = FOOD_COLOR
...
def divide_conquer():
    ...
    for point in points:
        r = 1.0
        draw.ellipse(
            (point[0] - r, point[1] - r, point[0] + r, point[1] + r), (255, 0, 0)
        )
...
def make_symmetric(points, size, players):
    ...
    if row_sym % 2 == 0:
        points, size = extend(
            choice((vert_copy, vert_mirror, vert_rotate)), points, size
        )
        row_sym /= 2
    if row_sym > 1:
    ...
    if col_sym % 2 == 0:
        points, size = extend(
            choice((horz_copy, horz_mirror, horz_rotate)), points, size
        )
        col_sym /= 2
    if col_sym > 1:
...
def cells(
    size, points, min_gap=5, max_braids=1000, openness=0.25, distance=euclidean_distance
):
    rows, cols = size
    size = (rows, cols)
            ...
            if len(closest) > 1:
                if comps_found:
                    ...
                    if len(nearest) == 2:
                        ...
                        m_row, m_col = mid_point(
                            points[nearest[0]], points[nearest[1]], size
                        )
                        if (
                            row_distance(m_row, row, rows) <= rows // 4
                            and col_distance(m_col, col, cols) <= cols // 4
                        ):
                ...
                if (
                    row_distance(row, points[nearest][0], rows) >= rows // 2
                    or col_distance(col, points[nearest][1], cols) >= cols // 2
                ):
