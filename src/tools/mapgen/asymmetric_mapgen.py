...
class AsymmetricMap:
    ...
    def pick_dimensions(self):
        while True:
            while True:
                ...
                if self.rows // gcd(self.row_t, self.rows) == self.cols // gcd(
                    self.col_t, self.cols
                ):
                    break
            ...
            if (
                self.no_players >= self.min_players
                and self.no_players <= self.max_players
    ...
    def get_translate_loc(self, loc):
        return [(loc[0] + self.row_t) % self.rows, (loc[1] + self.col_t) % self.cols]

    # returns the new location after translating it by (rtranslate, ctranslate)
    def get_translate_loc2(self, loc, row_t, col_t):
        return [(loc[0] + row_t) % self.rows, (loc[1] + col_t) % self.cols]
    ...
    def is_valid(self):
        ...
        while stack != []:
            ...
            for d in self.directions:
                ...
                if (
                    visited[n_loc[0]][n_loc[1]] == False
                    and self.map_data[n_loc[0]][n_loc[1]] != "%"
                ):
                    stack.append(n_loc)
    ...
    def add_random_land(self):
        no_land_squares = random.randint(
            int(self.min_land_proportion * self.rows * self.cols),
        ...
        while self.land_squares < no_land_squares or not self.is_valid():
            for _ in range(self.no_extra_walks):
                for n in range(self.no_players):
                    ...
                    c_loc = self.get_translate_loc2(
                        c_loc, n * self.row_t, n * self.col_t
                    )
